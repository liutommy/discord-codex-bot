...
# OpenRouter is stateless: the Bot keeps each conversation itself, under this thread-id prefix.
THREAD_PREFIX = "or-"
RETRY_AFTER_FAILURE_SECONDS = 60
UNSTABLE = "（免費模型可能暫時不可用，換一個試試）"
# OpenRouter's reasoning.effort vocabulary; the shared option's "max" is its "xhigh".
_EFFORT = {"low": "low", "medium": "medium", "high": "high", "xhigh": "xhigh", "max": "xhigh"}
# Free entries that are not chat models for our purpose (probed 2026-09-12): safety classifiers
# answer "User Safety: safe", and the inkling models refuse plain chat (403 "agentic harnesses").
_EXCLUDED = ("content-safety", "thinkingmachines/inkling")
...
def parse_catalog(payload: dict) -> list[Model]:
    ...
    for entry in payload.get("data") or []:
        ...
        if pricing.get("prompt") != "0" or pricing.get("completion") != "0":
            continue
        if any(marker in str(entry["id"]) for marker in _EXCLUDED):
            continue
...
def _session(config: Config, timeout_seconds: int) -> aiohttp.ClientSession:
...
class Catalog:
    """The free-model list, refreshed from the API at most once per TTL. Free models come and go,
    so the API is the only source of truth; the last good list survives a failed refresh."""
    ...
    async def free_models(self) -> list[Model]:
        ...
        async with self._lock:
            if self._fresh():
                return self.models
            try:
                async with _session(self._config, self._config.link_timeout_seconds) as session:
                    async with session.get(f"{API}/models") as response:
                        response.raise_for_status()
                        payload = await response.json(content_type=None)
                self.models = parse_catalog(payload)
                self._at = time.monotonic()
            except (aiohttp.ClientError, TimeoutError, ValueError, KeyError) as error:
                LOGGER.warning("OpenRouter catalog refresh failed: %s", type(error).__name__)
...
async def run_openrouter(
    ...
    """One turn on OpenRouter with the same contract as run_codex. The conversation lives in a
    Bot-kept transcript (thread id `or-…`); `resume` replays it, bounded by
    OPENROUTER_HISTORY_CHARS. Images go inline (base64) when the catalog says the model reads
    them; effort is sent only when the catalog says the model takes it."""
    ...
    try:
        async with _session(config, config.codex_timeout_seconds) as session:
            async with session.post(f"{API}/chat/completions", json=body) as response:
                status = response.status
                payload = await response.json(content_type=None)
    except (aiohttp.ClientError, TimeoutError, ValueError) as error:
        raise RuntimeError(f"OpenRouter 連線失敗：{type(error).__name__}{UNSTABLE}") from error
    ...
    if status != 200 or error:
        ...
        raise RuntimeError(f"OpenRouter {where}：{detail or '無說明'}{UNSTABLE}")
    choices = payload.get("choices") or []
    text = message_text((choices[0].get("message") or {}).get("content") or "") if choices else ""
    if not text.strip():
        raise RuntimeError(f"OpenRouter 沒有回傳文字{UNSTABLE}")
